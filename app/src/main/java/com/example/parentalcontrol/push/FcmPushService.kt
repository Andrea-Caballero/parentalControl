...
class FcmPushService {
    ...
    companion object {
        ...
        const val ACTION_FCM_MESSAGE = "com.example.parentalcontrol.FCM_MESSAGE"
...
class FcmTokenWorkerStub(
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            val clientProvider = com.example.parentalcontrol.network.SupabaseClientProvider.getInstance(applicationContext)
    ...
    private suspend fun enqueueTokenToOutbox(token: String) {
        try {
            val syncManager = com.example.parentalcontrol.sync.SyncManager.getInstance(applicationContext)
            syncManager.enqueue(
                com.example.parentalcontrol.sync.OutboxEventType.HEARTBEAT,
                mapOf(
...
class FcmSyncWorkerStub(
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            ...
            when (result) {
                is com.example.parentalcontrol.sync.SyncResult.Success -> {
                    Log.d(TAG, "Sync completado exitosamente")
                    Result.success()
                }
                is com.example.parentalcontrol.sync.SyncResult.PartialSuccess -> {
                    Log.w(TAG, "Sync parcial: ${result.failedItems} items fallidos")
                    Result.success()
                }
                is com.example.parentalcontrol.sync.SyncResult.Offline -> {
                    Log.d(TAG, "Offline, reintentando...")
                    Result.retry()
                }
                is com.example.parentalcontrol.sync.SyncResult.Error -> {
