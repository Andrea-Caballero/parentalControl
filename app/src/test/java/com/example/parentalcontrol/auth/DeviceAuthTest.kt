...
class ConnectionStateTest {
    ...
    fun `connectionState enum has correct values`() {
        val states = com.example.parentalcontrol.network.ConnectionState.entries.toTypedArray()
        assertEquals(6, states.size)
        assertTrue(states.contains(com.example.parentalcontrol.network.ConnectionState.DISCONNECTED))
        assertTrue(states.contains(com.example.parentalcontrol.network.ConnectionState.CONNECTING))
        assertTrue(states.contains(com.example.parentalcontrol.network.ConnectionState.CONNECTED))
        assertTrue(states.contains(com.example.parentalcontrol.network.ConnectionState.NEEDS_PAIRING))
        assertTrue(states.contains(com.example.parentalcontrol.network.ConnectionState.INTEGRITY_ERROR))
        assertTrue(states.contains(com.example.parentalcontrol.network.ConnectionState.ERROR))
    }
}
