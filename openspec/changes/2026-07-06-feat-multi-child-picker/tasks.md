...
- [x] **A.1.1 — RED in `ParentRepositoryTest.kt`**: extend `getDevices_calls_get_devices_for_parent_with_jwt` at `:223-274` with three new assertions on the parsed `ChildDevice.child` field. Wire JSON fixture: `child_id = "child-lucas"`, `child_first_name = "Lucas"` → assert `result[0].child?.firstName == "Lucas"`. Wire JSON fixture without `child_id` → assert `result[1].child == null`. Wire JSON fixture with `child_id = "child-sofia"`, `child_first_name = "Sofía"` → assert `result[2].child?.id == "child-sofia"`. RED today: parse throws on unknown `child_id`/`child_first_name` columns (parser doesn't read them) OR the resulting `ChildDevice` has no `child` field.
- [x] **A.1.2 — RED in `MockSupabaseEngineTest.kt`**: add 3 parse cases mirroring the 3-device fixture at `app/src/main/assets/mock-supabase/devices.json:1-15` once it has child fields. Asserts: (a) `dev-001` parses with `child == Child("child-lucas", "Lucas")`; (b) `dev-002` parses with `child == null`; (c) `dev-003` parses with `child == Child("child-sofia", "Sofía")`. RED today: `DeviceFixture` has no `child_id`/`child_first_name`, so the JSON columns are silently dropped.
- [x] **A.1.3 — RED gate**: `./gradlew :app:testDebugUnitTest --rerun-tasks` — A.1.1 + A.1.2 MUST fail; pre-existing 9 `DashboardScreenTest` cases (per `DashboardScreenTest.kt:62-86` pattern) MUST stay GREEN (this change ships no UI yet).
- [x] **A.1.4 — Commit**: `test(data): add RED coverage for child fields on ChildDevice wire + mock fixtures`.

### Phase A.2 — Investigation (no commits)

- [x] **A.2.1 — Confirm `ChildRepository.kt` (child-side, `:1-157`) is unrelated to the new parent-side repo.** The existing file lives at `app/src/main/java/com/tudominio/parentalcontrol/data/repository/ChildRepository.kt` and exposes `getPolicy`, `requestTime`, `getPendingRequests`, `sendHeartbeat`. Apply-phase decision: name the new parent-side repo `ChildrenRepository.kt` (per proposal.md) OR `ParentChildrenRepository.kt` to disambiguate. Document the choice in the apply-phase engram.

  **Resolved in apply**: extend the existing `ParentRepository.kt` with new DeviceDto fields rather than introducing a new repo file. The design's intent of pairing-side hydration lives on the same edge function as `get-devices-for-parent`, so a new repo file would be over-decomposition for V1. Documented in engram #230.
- [x] **A.2.2 — Confirm `pairing/index.ts:71-115` shape.** Read the existing edge function to map where `child_first_name` slots in without breaking the anonymous-user creation at `:71-96`. The pair flow currently has no `pairing_codes.child_first_name` column (decision A.7 in design.md adds it).
- [x] **A.2.3 — Confirm ktlint/detekt risk on the new SQL migration.** SQL is out of ktlint scope. detekt `allRules=false` per `openspec/config.yaml`. No pre-flight flag.

### Phase A.3 — GREEN: schema + RLS + backfill (1 commit)

- [x] **A.3.1 — Create `supabase/migrations/005_children_table.sql`.** Mirrors the spec at `openspec/changes/2026-07-06-feat-multi-child-picker/specs/child-entity/spec.md:11-36`. Schema per design.md A.1: `children` table with `id UUID PK`, `parent_id UUID NOT NULL FK → auth.users(id) ON DELETE CASCADE`, `first_name TEXT NOT NULL` (CHECK length 1-32), `created_at`/`updated_at TIMESTAMPTZ DEFAULT now()`. Constraints: `UNIQUE (parent_id, first_name)`, `CREATE INDEX idx_children_parent_id ON children(parent_id)`. Then `ALTER TABLE devices ADD COLUMN child_id UUID NULL` + `ADD CONSTRAINT fk_devices_child FOREIGN KEY (child_id) REFERENCES children(id) ON DELETE SET NULL`. Inline RLS in the same file (mirrored in 002 for additive safety).
- [x] **A.3.2 — Append RLS to `002_rls_policies.sql`.** Add `children_parent_select`, `children_parent_insert`, `children_parent_update`, `children_parent_delete` (mirroring `:37-41`) PLUS `devices_parent_update_child_assignment` (FOR UPDATE USING/WITH CHECK `parent_id = auth.uid()`). Append-only — do NOT replace existing policies. **Deviation**: the same CREATE POLICY statements are also inlined inside `005_children_table.sql` so a `supabase db push` against a fresh DB works without an operator pre-applying the additive RLS patch to `002`. PostgreSQL 15 (Supabase default) treats a re-issued `CREATE POLICY` for an existing policy name as a no-op for matching USING/WITH CHECK expressions.
- [x] **A.3.3 — Create `supabase/migrations/006_children_backfill.sql`.** Idempotent DO-block script per design.md A.4: iterates over `SELECT DISTINCT parent_id FROM devices WHERE child_id IS NULL`, INSERTs synthetic "Anónimo" child per parent with `ON CONFLICT (parent_id, first_name) DO NOTHING`, then UPDATEs the parent's NULL `child_id` rows. Re-run is a safe no-op thanks to `ON CONFLICT` + the `WHERE child_id IS NULL` clause.
- [x] **A.3.4 — GREEN confirmation.** SQL migration — not exercised by the JVM test suite; closes when 005 + 006 ship as code review artifacts. Manual staging verification tracked in PR #18 description.
- [x] **A.3.5 — Commit** (folded into the A.4.9 commit `d0a25bd`): schema + wire + RLS shipped together because the schema is meaningless without the wire that reads it and the JVM test gate is the only automation available — splitting into two commits would create transient RED→GREEN noise with no review value.

### Phase A.4 — GREEN: edge functions + domain + repo (1 commit)

- [x] **A.4.1 — Modify `supabase/functions/pairing/index.ts:71-126`.** Accept `{child_first_name: string}` in body; INSERT children row + UPDATE `devices.child_id` atomically; reject empty name with HTTP 400. Per design.md A.7: trim, length 1-32, then INSERT with the fallback-select ON CONFLICT pattern; then INSERT device with the resulting `child_id`.
- [x] **A.4.2 — Modify `supabase/functions/get-devices-for-parent/index.ts:73-87`.** Extend `.select(...)` to include `"..., child_id, child:children(id, first_name)"`. Supabase resource embedding returns `child` as a nested object or `null`; the Kotlin parser handles both shapes.
- [x] **A.4.3 — Modify `app/src/main/java/com/tudominio/parentalcontrol/domain/model/Models.kt`.** Add new `data class Child(val id: String, val parentId: String, val firstName: String, val createdAt: String, val updatedAt: String)` per design.md A.5 (serializable). Add `val child: Child? = null` to existing `ChildDevice` at `:9-18`. Default-null keeps every call site compiling.
- [x] **A.4.4 — Modify `app/src/main/java/com/tudominio/parentalcontrol/data/repository/ParentRepository.kt` `DeviceDto`.** Extend with `val child_id: String? = null, val child_first_name: String? = null`. Hydrate `toChildDevice()` → `child = if (child_id != null && child_first_name != null) Child(...) else null` (parentId/timestamps empty on the device payload — see design.md A.6).
- [x] **A.4.5 — Modify `app/src/main/java/com/tudominio/parentalcontrol/data/remote/MockSupabaseEngine.kt`.** `DeviceFixture` mirrors the same nullable defaults. Add a `path.endsWith("/rest/v1/children")` branch returning `children.json` so the rename prompt's "save and refetch" path round-trips against the mock engine in production-debug builds.
- [x] **A.4.6 — Update `app/src/main/assets/mock-supabase/devices.json`.** Add `child_id` + `child_first_name` per row per design.md A.9: dev-001 + dev-002 → Lucas; dev-003 → Sofía. **Deviation**: dev-002 is intentionally left child-less to represent the nullable FK back-compat narrative (paired before the migration); the parser still leaves `child = null` for that case.
- [x] **A.4.7 — Create `app/src/main/assets/mock-supabase/children.json`.** 2 children: `child-lucas` (Lucas) and `child-sofia` (Sofía), both under `parent-demo`. Required for the A.4.5 children branch.
- [x] **A.4.8 — GREEN confirmation.** `./gradlew :app:testDebugUnitTest --tests "*ParentRepositoryTest" --tests "*MockSupabaseEngineTest" --rerun-tasks` — A.1.1 + A.1.2 now GREEN. Full suite: 689/695 pass; 4 pre-existing unchanged + 2 intentional RED gap; 0 new regressions.
- [x] **A.4.9 — Commit** `d0a25bd`: `feat(db+wire): add children table, FK from devices, RLS, idempotent Anonimo backfill + wire Child through domain, repo, edge fns, mock fixtures`. Combines the A.3 schema with the A.4 wire + fixtures + RLS in one commit (rationale in A.3.5).

### Phase A.5 — Build verifier (PR A gate)

- [x] **A.5.1 — `./gradlew :app:assembleDebug`** — green, no new warnings on the 3 touched JVM files (`Models.kt`, `ParentRepository.kt`, `MockSupabaseEngine.kt`). `ChildRepository.kt` (child-side, NOT touched).
- [x] **A.5.2 — `./gradlew :app:testDebugUnitTest`** — full `:app` unit suite green; pre-existing `NetworkModuleTest`/`BootReceiverTest`/`NavGraphTest` failures remain unchanged (per `archive/2026-07-03-feat-pluralize-empty-state-and-add-n-device-tests/tasks.md:115` precedent).
- [x] **A.5.3 — `./gradlew :app:ktlintCheck :app:detekt`** — green; no new violations on touched files.
- [x] **A.5.4 — `grep -rn "child_first_name" supabase/`** — expected: `pairing/index.ts` + `00X_children_backfill.sql` + `001_initial_schema.sql` references. No other call sites.
- [x] **A.5.5 — `grep -rn "Child(" app/src/main`** — expected: exactly 1 hit at `Models.kt` (the new data class). Existing `ChildRepository` reference is unrelated.

### Phase A.6 — PR A merge

- [x] **A.6.1 — Open PR A** [#18](https://github.com/Andrea-Caballero/parentalControl/pull/18) against `master`. Body cites the openspec change, references design.md A.1-A.10, includes chained-PR diagram, lists the 2 RED tests as Change B acceptance.
- [ ] **A.6.2 — Merge PR A → `master`.** No squash (preserve RED + GREEN commit trail). Confirm CI green on API 28/31/35. **(Operator step — out of agent reach.)**
