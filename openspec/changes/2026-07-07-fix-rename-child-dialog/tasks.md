...
- **Branch:** `fix/rename-child-dialog` based on `master @ 9b57669`.
...
- **Work units shipped (3 commits):**
  ...
  2. `feat(rename-child): add Material 3 rename modal with pessimistic PATCH flow` — new `RenameChildDialog.kt` + `ParentRepository.renameChild` + `MockSupabaseEngine` PATCH handler + `ParentViewModel.RenameChildState` + `DashboardScreen` wiring + `DeviceComponents` long-press/overflow menu; turns all 11 RED tests GREEN.
  3. `chore(openspec): mark apply tasks complete in tasks.md` — this file.
- **Final test totals (`./gradlew :app:testDebugUnitTest`):** TBD by apply phase. All 11 new tests GREEN; the 4 pre-existing failures (`NetworkModuleTest` + 2× `BootReceiverTest` + `NavGraphTest`) unchanged — no new regressions.
- **RED → GREEN:** T1.1, T1.2, T1.3, T1.4, T1.5, T1.6, T1.7, T1.8, T1.9, T1.10, T1.11.
- **Lint / detekt:** clean on the 6 touched production files + 3 new test files. The pre-existing 11+ main-source-set ktlint violations are out of scope per Phase 4.3.
