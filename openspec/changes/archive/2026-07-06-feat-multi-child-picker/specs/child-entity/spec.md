...
The release SHALL include a one-shot backfill script that runs against the production database BEFORE the new dashboard ships, so every pre-existing `devices.child_id` is non-NULL. The script's owner-strategy is **[NEEDS DECISION: synthetic Anónimo owner vs prompt-per-device]** — the spec phase deliberately leaves this unresolved. The script SHALL be idempotent so it can be re-run safely.
...
- **THEN** every `devices.child_id` that was NULL SHALL be set to the id of the chosen owner-child (synthetic or per-device-prompted), the chosen strategy SHALL be recorded in the script's audit log, and the script SHALL exit 0.

#### Scenario: A re-run is a safe no-op
- **WHEN** the operator runs the backfill script a second time,
- **THEN** no `devices.child_id` SHALL be overwritten and the script SHALL exit 0.
